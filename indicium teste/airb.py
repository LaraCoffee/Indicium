...
preco_medio = df.groupby(['bairro', 'bairro_group']).agg(media_preco=('price', 'mean'),count=('id', 'size')).reset_index()
preco_medio['media_preco'] = preco_medio['media_preco'].round(2)
# preco_medio = preco_medio.sort_values(by='price', ascending=False)

preco_medio.columns = ['bairro', 'distrito','preco_medio', 'quantidade de ocorrencias']
...
#grafico com minimo de noites e disponibilidade x preço
dispersao_noites_preco = px.scatter(df, x='minimo_noites', y='price', color='disponibilidade_365', title='Relação entre o Minimo de Noites, Preço e Disponibilidade',
    labels={'minimo_noites': 'Mínimo de Noites', 'price': 'Preço', 'disponibilidade_365': 'Disponibilidade (365)'}, color_continuous_scale='sunset')
...
with col2:
    st.plotly_chart(dispersao_noites_preco)
with col3:
...
#modelo de prediçao ja treinado 
